...
To use GitReflow with a GitHub Enterprise account, you will need to add some additional git configuration settings.

If you use a GitHub Enterprise account for the majority of your repositories, you'll want to setup these global git config settings:

  git config --global github.endpoint "https://github.company.com/api/v3"
  git config --global github.site "https://github.company.com"

And in the future, if you want to use a regular Github account for a certain project, just `cd` into your project directory and run the folowing setup command:
...
If you only use GitHub Enterprise for a select few repositories, then you'll want to add these git config settings to every project that uses GitHub Enterprise:
