...
Feature: User starts a new flow
  ...
  Scenario: User runs git-reflow start with new branch name
    ...
    Then the output should contain "-- Using the default base branch 'master'"
  ...
  Scenario: User runs git-reflow start with new branch name and origin branch name
    Given I have a git repository with a branch named "main"
    And I have a remote git repository named "origin"
    And the remote repository named "origin" has changes on the "master" branch
    And I cd to "master_repo"
    When I run `git-reflow start new-branch --base=main`
    Then the output should match /\* \[new branch\]\s* main\s* \-\> new\-branch/
    And the output should match /\* \[new branch\]\s* main\s* \-\> origin.main/
    And the output should contain "Switched to a new branch 'new-branch'"
